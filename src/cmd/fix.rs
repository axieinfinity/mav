...
pub fn get_command<'a>() -> Command<'a, str> {
    Command::new(
        file_stem!(),
        "Fixes local network for the development Minikube machine",
        |app| app,
        run,
    )
}

pub fn run(env: &str, _matches: &ArgMatches<'_>) {
    if env != "dev" {
        ...
        util::Os::MacOs => {
            ...
            if util::get_minikube_status() == util::MinikubeStatus::Running {
                ...
                if netstat
                    ...
                    .is_some()
                {
                    util::Command::new("sudo", vec!["route", "-n", "delete", "10.96/12"]).run();
                }

                if netstat.find(|&line| line.starts_with("172.17 ")).is_some() {
                    util::Command::new("sudo", vec!["route", "-n", "delete", "172.17/16"]).run();
                }
